...
export class WhatsappStorageService {
  ...
  constructor(
    @InjectModel(WhatsAppChat.name, 'conn2') private whatsAppChatModel: Model<WhatsAppChatDocument>,
    @InjectModel(WhatsAppMessage.name, 'conn2') private whatsAppMessageModel: Model<WhatsAppMessageDocument>,
  ) {}
  ...
  async saveMessage(sessionId: string, message: WAWebJS.Message, chatId?: string): Promise<void> {
    try {
      ...
      if (!existingMessage) {
        await this.whatsAppMessageModel.create(messageData);
        this.logger.debug(`💾 Message saved: ${message.id._serialized}`);
      } else {
